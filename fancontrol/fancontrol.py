...
def configure():
    ...
    try:
        ...
        logger.exception("can't load config.yaml", extra=err)
        sys.exit(5)
...
def main():
    ...
    try:
        ...
        for x in range(1, 10):
            ...
            logger.info("fanspeed", extra={
                "readings": get_sensor_readings(ipmi, fanspeed_sensor_ids)
            })
            logger.info("cpu_temp", extra={
                "readings": get_sensor_readings(ipmi, cpu_temp_sensor_ids)
            })
            logger.info("ambient_temp", extra={
                "readings": get_sensor_readings(ipmi, ambient_temp_sensor_ids)
            })
